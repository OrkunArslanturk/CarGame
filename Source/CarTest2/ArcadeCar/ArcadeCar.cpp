...
void AArcadeCar::Tick(float DeltaTime)
{
	...
	if (RacingSpline)
	{
		...
		float SplineLength = RacingSpline->Spline->GetSplineLength();

		float Normalized = 0.f;

		if (SplineLength > 0.f)
		{
			Normalized = DistanceOnSpline / SplineLength;
		}

		// FINAL PROGRESS
		Progress = (CurrentLap * 1000.f) + (CurrentCheckpoint * 100.f) + Normalized;
	}
...
void AArcadeCar::ShowDebugInfo()
{
	...
	GEngine->AddOnScreenDebugMessage(0, 0.f, FColor::Green,
	                                 FString::Printf(
		                                 TEXT("Speed: %.0f km/h | Gear: %s"), FMath::Abs(SpeedKMH), *GearStr));
	GEngine->AddOnScreenDebugMessage(1, 0.f, NitroColor,
	                                 FString::Printf(
		                                 TEXT("NITRO %s %.0f%% %s"), *NitroBar, NitroPercent * 100.f,
		                                 bIsNitroActive ? TEXT("BOOST!") : TEXT("")));
}
